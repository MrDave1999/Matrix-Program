...
#line 163 "c:\\users\\mrdave\\desktop\\prueba\\include/op.h"
#line 7 "main.c"
...
uint8_t DataEntry(void)
{
	...
	printf("Ingrese el numero de filas de la Matriz A"); puts(""); _datareadEx("%d", &dim[MA].new_rows);
	printf("Ingrese el numero de columnas de la Matriz A"); puts(""); _datareadEx("%d", &dim[MA].cols);
	
	printf("Ingrese el numero de filas de la Matriz B"); puts(""); _datareadEx("%d", &dim[MB].new_rows);
	printf("Ingrese el numero de columnas de la Matriz B"); puts(""); _datareadEx("%d", &dim[MB].cols);
	...
	{
		for(j = 0; j != dim[MA].cols; ++j)
			(*((int*)*(*(buf_ptr + MA) + i) + j)) = ((rand() % (5-(-10)+1)) + (-10));
	}
	...
	for(i = 0; i != dim[MB].new_rows; ++i)
	{
		for(j = 0; j != dim[MB].cols; ++j)
			(*((int*)*(*(buf_ptr + MB) + i) + j)) = ((rand() % (5-(-10)+1)) + (-10));
	}
	return 0;
}

#line 38 "c:\\users\\mrdave\\desktop\\prueba\\include/dataentry.h"
#line 8 "main.c"
...
int mDynamic_main(void) 
{
	...
	while(1)
	{
		...
		printf("Ingrese una opcion <1-8>"); puts(""); _datareadEx("%c", &option);
		if(!!!(!(!(option >= '1' && option <= '8')))) { printf("\nError: Ingrese una opcion valida.\n"); printf("\n"); continue; };
		switch(option)
		{
			case '1':
				if(DataEntry() == 1) return 1;
				as_Matrix(ADD);
				break;
			case '2':
				if(DataEntry() == 1) return 1;
